...
import { useRaaspayContract } from "./useRaaspayContract";

export default function RaaspayTradingPanel() {
  const RaaspayContract = useRaaspayContract();
  ...
  const loadMarketData = async () => {
    try {
      setLoading(true);
      if (RaaspayContract) {
        const data = await RaaspayContract.getOrderBook(); // adjust as needed
        setMarketData(data);
      } else {
        setError("⚠️ Raaspay contract not loaded");
      }
    } catch (err) {
  ...
  const handleTrade = async (type) => {
    try {
      ...
      const tx = type === "buy"
        ? await RaaspayContract.placeBid(value)
        : await RaaspayContract.placeAsk(value);
  ...
  useEffect(() => {
    if (RaaspayContract && connected) {
      loadMarketData();
    }
  }, [RaaspayContract, connected]);

  return (
    <div className="p-6 bg-gray-900 text-white rounded-2xl shadow-lg max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-yellow-400">📈 Raaspay Trading Panel</h2>
